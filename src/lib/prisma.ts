...
function ensureEnv(name: 'DATABASE_URL_WRITE' | 'DATABASE_URL_READ'): string {
  ...
  if (!value) {
    throw new Error(`Variável de ambiente ${name} não definida. Configure-a no arquivo .env`);
  }
  return value;
}
...
export const prisma: PrismaReadWrite = globalForPrisma.prismaRW ?? createClients();
